...
const CollectionDisplay: React.FC<CollectionDisplayProps> = ({ collection, lang, onEnhance, isEnhancing }) => {
  ...
  return (
    <div className="w-full animate-fade-in mt-12 pb-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 border-b border-gray-800 pb-4 gap-4">
        ...
        <div className="flex gap-3">
             {onEnhance && (
                <button
                    ...
                    className={`px-6 py-2 rounded text-xs font-bold font-mono tracking-widest transition-all flex items-center gap-2 border 
                    ${isEnhancing ? 'bg-purple-900/30 border-purple-500 text-purple-200 cursor-not-allowed' : 'bg-gray-900 border-purple-500/50 text-purple-400 hover:bg-purple-900/20 hover:border-purple-400 hover:shadow-[0_0_15px_rgba(168,85,247,0.2)]'}`}
                >
                    {isEnhancing ? (
                        <>
