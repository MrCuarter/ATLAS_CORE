...
/**
 * Detects thematic clashes between selected style and civilization.
 */
const detectClash = (style: string, civ: string): string | null => {
  ...
  const sciFiStyles = ['cyber', 'neon', 'mass effect', 'halo', 'destiny', 'starcraft', 'matrix', 'dune', 'blade runner', 'tron', 'sci-fi', 'robotic', 'futuristic'];
  const historicalCivs = ['medieval', 'viking', 'roman', 'ancient', 'tribal', 'prehist', 'renaissance', 'middle ages', 'elfos', 'orcos', 'enanos', 'piratas', 'antigua', 'feudal', 'edad media'];
  const stylizedStyles = ['anime', 'cartoon', 'ghibli', 'pixar', 'disney', 'simpsons', 'rick and morty', 'adventure time', 'fortnite', 'mario', 'pokemon', 'zelda', 'overwatch', 'watercolor', 'voxel', 'pixel art'];
  const grimCivs = ['dark fantasy', 'horror', 'lovecraft', 'terror', 'no-muertos', 'undead', 'demon', 'post-apocalyptic', 'realista', 'bloodborne', 'dark souls', 'elden ring', 'vampire', 'necromancer', 'grim'];

  const isSciFi = sciFiStyles.some(keyword => s.includes(keyword));
  const isHistorical = historicalCivs.some(keyword => c.includes(keyword));
  const isStylized = stylizedStyles.some(keyword => s.includes(keyword));
  const isGrim = grimCivs.some(keyword => c.includes(keyword));

  if (isSciFi && isHistorical) return "ANACHRONISM_WARNING";
  if (isStylized && isGrim) return "TONE_CLASH_WARNING";
...
const StyleSelector: React.FC<StyleSelectorProps> = ({ selectedStyle, civilization, onSelect, lang, isSimpleMode = false }) => {
  ...
  const getItems = (cat: StyleCategory) => {
    ...
    // Increased limit for simple mode as requested
    if (isSimpleMode) {
          ...
          return (
              <div className="mb-3 p-2 bg-yellow-900/20 border border-yellow-600/50 rounded flex items-start gap-2 animate-fade-in">
                  ...
                  <div>
                      ...
                      <p className="text-[10px] text-yellow-200/70 leading-tight">Estilo <strong>{selectedStyle}</strong> + <strong>{civilization}</strong>.</p>
                  </div>
              </div>
          );
      }
        ...
        return (
            <div className="mb-3 p-2 bg-pink-900/20 border border-pink-600/50 rounded flex items-start gap-2 animate-fade-in">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-pink-200/70 leading-tight">Estilo <strong>{selectedStyle}</strong> + <strong>{civilization}</strong>.</p>
                </div>
            </div>
        );
      }
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-3">
          ...
          {selectedStyle && <span className="text-[9px] font-mono text-gray-500 uppercase bg-gray-900 px-2 py-0.5 rounded border border-gray-800">ACTIVO</span>}
      </div>

      {renderWarning()}

      <div className="flex bg-gray-950 p-1 rounded border border-gray-800 mb-4">
        {['ART', 'GAME', 'MEDIA'].map((cat) => (
            <button
            key={cat}
            onClick={() => { setActiveCategory(cat as StyleCategory); playTechClick(); }}
            ...
            >
            {cat === 'ART' ? t.styleArt : cat === 'GAME' ? t.styleGame : t.styleMedia}
            </button>
        ))}
      </div>

      <div className={`bg-gray-950 border border-gray-800 rounded p-2 custom-scrollbar ${isSimpleMode ? 'h-auto' : 'h-64 overflow-y-auto'}`}>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {currentItems.map((style) => (
              <button
                ...
                `}
              >
                {selectedStyle === style && <div className="absolute top-1 right-1 w-1.5 h-1.5 bg-purple-400 rounded-full shadow-[0_0_5px_rgba(168,85,247,0.8)]"></div>}
                {style}
              </button>
          ))}
        </div>
